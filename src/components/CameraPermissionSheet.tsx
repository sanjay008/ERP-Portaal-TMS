...
import type { CameraAccessStatus } from '@/src/hooks/useCameraPermission';
...
export default function CameraPermissionSheet({
  visible,
  reason,
  loading = false,
  onClose,
  ...
  const content = useMemo((): SheetContent | null => {
    ...
    if (reason === 'blocked') {
      return {
        title: t('Camera access is turned off'),
        description: t(
          'Camera permission is required to scan QR codes. Enable it in your device settings to continue.',
        ),
        steps: [
          t('Open Settings'),
          t('Tap Permissions → Camera'),
          t('Allow camera access for this app'),
        ],
    ...
    return {
      ...
      steps: [
        t('Tap "Allow Camera" below'),
        t('Choose "Allow" on the system prompt'),
      ],
      primaryLabel: t('Allow Camera'),
      accent: Colors.green,
    };
  }, [reason, t]);
      ...
      >
        ...
        {reason === 'denied' ? (
          <Pressable
            ...
            disabled={loading}
          >
            <Text style={styles.settingsBtnText}>{t('Open Settings')}</Text>
          </Pressable>
        ) : null}
