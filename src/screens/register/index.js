  ...
  const onRegisterCompany = async () => {
    ...
    try {
      ...
      if (data?.status) {
        ...
        const googleKey = data?.data?.default_company?.project_google_maps_api_key;
        storeData("COMPANYLOGO", logoUrl);
        setCompanyLogo(logoUrl);
        await storeData("google_maps_api_key", googleKey);
        storeData("GOOGLE_API_KEY", googleKey);
        setcompanylogo(logoUrl);
        setLogo(logoUrl);
        setTimeout(() => {
          setLoding(false);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.container}>
          ...
          <>
            {complate == false ? (
              <Input
                value={commpny}
                onChangeText={(txt) => { setcommpny(txt); setCompnyError(""); }}
                title={t("company")}
                error={CompnyError}
        ...
        {typee === "Login" && complate == true && showWhatsApp && !showEmail && (
          <Text
            onPress={() => {
              setShowEmail(true);
