...
export default function BottomTabs() {
  ...
  const { Permission, setPermission,UserData,setUserData,setCompanysData } = useContext(GlobalContextData);
  // Permission?.tms_driver_loaded_and_parcel_menu?.read
  useEffect(() => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white }}>
      <Tab.Navigator
        initialRouteName={Permission?.tms_driver_loaded_and_parcel_menu?.read == "1" ? "Loaded" : "Profile"} 
        screenOptions={{
          tabBarStyle: {
            height: height * 0.09,
